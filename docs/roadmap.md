...
| 指标 | v1.0 | v2.0 | v3.0 |
|------|------|------|------|
| 单次记账操作步骤 | 7 步 | 5 步 | 3 步 (OCR 拍照自动填充) |
| 仪表盘信息维度 | 3 数字 + 1 表格 | 3 数字 + 2 图 + 1 日历 | 3 图 + 报表下载 + 超预算预警 |
| 数据导出方式 | 仅后端 API | 前端一键导出 (CSV/JSON) | + PDF 报表下载 |
| 自动汇率 | 手动录入 | 手动录入 | 自动每日更新 (UTC 02:00) |
| 移动端支持 | 仅响应式 | 仅响应式 | PWA 离线可用 + 桌面图标 |
| 周期性交易 | — | — | 每日/每周/每月/每年自动生成 |
